...
export default function UploadComponent({ UploadDescription }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/images', galleryFormData, {
        headers: {
        ...
        <div className="event-popup">
          <form onSubmit={handleSubmit}>
            <div className="event-popup-content">
              ...
              <button className="upload-image-button" type="button" onClick={handleButtonClick}>
                {t("upload-image")}
              </button>
